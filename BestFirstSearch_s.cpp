...
void *Node_Processing(void *n)
{
    ...
    if(v->bound > lowerBound){ // else, prune by bound, do nothing  
        ...
        if(isFeasible && (u.bound > lowerBound)){  // if LP feasible and bound satisfies, else, prune 
        ...
        if(isFeasible && (w.bound > lowerBound)){ // if LP feasible and bound satisfies, else, prune
...
{ 
    ...
    while(!queue.empty()){ // when the priority queue is empty, we have the solution
        ...
        n = queue.back();
        queue.pop_back();
        Node_Processing((void *)&n);
    }
    time(&end);
    endTime = clock();
    cout << "SEQ: The node processing's real time is: " << setprecision(6) << difftime(end, start) << " secs";
